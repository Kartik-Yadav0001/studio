    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {resources.map((resource) => (
          <div key={resource.id} className="flex items-start justify-between p-3 rounded-lg bg-card-foreground/5 dark:bg-card-foreground/10 border">
            <div>
              <p className="font-semibold">{resource.id}</p>
              {resource.lockedByThreadId !== null ? (
                <p className="text-sm text-chart-4">Locked by Thread {resource.lockedByThreadId}</p>
              ) : (
            ...
            {resource.lockedByThreadId !== null ? (
              <Lock className="h-5 w-5 text-chart-4 flex-shrink-0" />
            ) : (
              <Unlock className="h-5 w-5 text-accent flex-shrink-0" />
            )}
          </div>
        ))}
